...
def modelo_LotkaVolterra(vetor_condicao_inicial, t, a, b, c, d):
    #x -> numero de presas
...
chutes = [param_a, param_b, param_c, param_d]

parametros_ajustados = leastsq(residual, chutes)

#Parametros ajustados
_param_a = parametros_ajustados[0][0]
_param_b = parametros_ajustados[0][1]
_param_c = parametros_ajustados[0][2]
_param_d = parametros_ajustados[0][3]

print(f'Parametros ajustados\n >a: {_param_a:.3f}\n >b: {_param_b:.3f}\n >c: {_param_c:.3f}\n >d: {_param_d:.3f}')

resultado = odeint(modelo_LotkaVolterra, condicao_inicial, dados_tempo, args=(_param_a, _param_b, _param_c, _param_d))
